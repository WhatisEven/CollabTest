...
public class ObjectManager extends Actor
{
        ...
        for (int i = 0; i < allObjects.size(); i++)
        {
            for (int f = 0; f < allObjects.get(i).size(); f++)
            {
                if (allObjects.get(i).get(f) != null)
                    ...
                    WorldGrid.addEmptyGrid(GameWorld.worldGrids[i][f]); // Adds the empty grid
            }
        }
    }
    ...
    {
        if (Greenfoot.getRandomNumber(100) < 60) // 60% for the current block to be a tree
        {
            return new Tree();
        }
        else if (Greenfoot.getRandomNumber(100) < 45) // 45% for the current block to be a rock
        {
            return new Rock();
        }
        return null;
    ...
    public static void removeObject(Actor obj)
    {
