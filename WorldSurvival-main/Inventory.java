...
public abstract class Inventory extends Actor
{
    ...
    public static void removeItem(int itemAmount, String itemName)
    {
        if (hasValidItem(itemAmount, itemName)) // Check if the player has the require amounts of item to be removed
        {
            ...
            {
                for (int f = inventoryItems[i].length - 1; f > 0 && Arrays.asList(inventoryItems[i]).contains(itemAmount) && itemAmount > 0; f--) // Loop through each individual column in the inventory backward 
                {
                    if (inventoryItems[i][f].contains(itemName)) // If the current slot has the item that it is currently looking for
                    {
                        itemAmount = itemAmount - ItemDataManager.getItemAmount(inventoryItems[i][f]);
                        inventoryItems[i][f] = (itemAmount > 0) ? null : ItemDataManager.getItemData(Math.abs(itemAmount), itemName);
                    }
                }
            }
        }
